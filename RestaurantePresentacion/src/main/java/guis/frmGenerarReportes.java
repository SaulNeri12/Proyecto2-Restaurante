...
public class frmGenerarReportes extends javax.swing.JFrame {
    ...
    private IReservacionesBO reservacionesBO;
    /**
     * Creates new form frmGenerarReportes
     */
    public frmGenerarReportes() {
 initComponents();
    this.reservacionesBO = new ReservacionesBO();
    cargarReservaciones(); 
    cargarEstados();
   ...
   private void filtrarReservacionesPorEstado(String estado) {
        ...
        for (ReservacionDTO reservacion : reservaciones) {
            if (reservacion.getEstado().name().equals(estado)) {
                modeloTabla.addRow(new Object[]{
                    ...
                    reservacion.getMesa().getCodigo(),
                    (reservacion.getMulta() != null) ? reservacion.getMulta().getPorcentaje() : 0
                });
            }
        }
    } catch (ServicioException ex) {
    ...
    private void initComponents() {
        ...
        tblResultado.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Estado", "Fecha", "MontoTotal", "Numero de personas ", "Cliente ", "Mesa", "Multa %"
            }
        ));
    ...
    try {
        ...
        if ("Sí".equals(selectedItem)) {
            ...
            reservaciones = reservacionesBO.obtenerReservacionesConMulta(); // Implementa este método para que devuelva todas las reservaciones con multa
        } else if ("No".equals(selectedItem)) {
            // Si se selecciona "No", obtenemos reservaciones sin multa
            reservaciones = reservacionesBO.obtenerReservacionesSinMulta(); // Implementa este método para que devuelva todas las reservaciones sin multa
        } else {
        ...
        for (ReservacionDTO reservacion : reservaciones) {
            modeloTabla.addRow(new Object[]{
                ...
                reservacion.getMontoTotal(),
                reservacion.getNumeroPersonas(),
                reservacion.getCliente().getTelefono(), // Número de teléfono del cliente
                reservacion.getMesa().getCodigo(),
                (reservacion.getMulta() != null) ? reservacion.getMulta().getPorcentaje() : 0
            });
        }
